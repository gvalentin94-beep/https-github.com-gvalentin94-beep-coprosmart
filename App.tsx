    ...
    return (
        <footer className="mt-20 border-t border-slate-800/50 py-8 text-center">
            <div className="flex items-center justify-center gap-2 mb-4 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
                ...
                <span className="font-bold text-white tracking-tight">CoproSmart <span className="text-indigo-500">v0.2.26</span></span>
            </div>
                        ...
                        <div className="grid gap-4">
                            {openTasks.map((t: Task) => (
                                <TaskCard 
                                    ...
                                    me={user} 
                                    usersMap={usersMap}
                                    onBid={(b: any) => onBidWrapper(t.id, b)} 
                        ...
                        <div className="grid gap-4 opacity-80 hover:opacity-100 transition-opacity">
                            {historyTasks.map((t: Task) => (
                                <TaskCard 
                                    ...
                                    onAward={() => {}} 
                                    onComplete={() => {}} 
                                    onRate={(r: any) => handleRate(t.id, r)}
                                    onDeleteRating={handleDeleteRating}
                                    onPayApartment={() => {}}
                                    onDelete={() => handleDeleteTask(t.id)}
                                    canDelete={user.role === 'admin'}
